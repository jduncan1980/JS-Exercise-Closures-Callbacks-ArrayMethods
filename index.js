 ...
 * counter1 is a variable containing a function expression created by calling the counterMaker function. it returns undefined because the inner function inside counterMaker doesn't have a return value (which I believe is probably a mistake in the original code).  counter2 is a function declaration created separately. it returns a count up (0, 1, 2, 3, etc.)

 * 2. Which of the two uses a closure? How can you tell?
 * They are both closures, because both functions are able to access  variables outside of their inner scope (counter1 is accessing the count variable in the outer scope of the countMaker function that was called to create it, and counter2 is accessing the count variable located in the global scope). Technically, all functions in javascript are closures.
 *
 * 3. In what scenario would the counter1 code be preferable? In what scenario would counter2 be better?
 * counter1 would be more useful in scenarios where you want to limit access to the counter variable to only the counterMaker function, and counter2 would be useful if you needed to access the count variable from another function.
 */
